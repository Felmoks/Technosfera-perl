...
sub pop_token {
    ...
    my $check = shift // token_type($token);
...
sub object {
    ...
    if (top_token() ne 'BRACE_RIGHT') {
        while (1) {
            ...
            last if top_token() eq 'BRACE_RIGHT';
            pop_token('COMMA');
        }
    }
...
sub array { 
    ...
    my @array;
    if (top_token() ne 'BRACKET_RIGHT') {
        while (1) {
            my $value = value();
            push @array, $value;
        } continue {
            last if top_token() eq 'BRACKET_RIGHT';
            pop_token('COMMA');
        }
    }
